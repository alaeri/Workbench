...
private fun FocusedAndZoomCommandHistory.toVM(
    ...
    val before = if(beforeCount > 0){
        FocusCommandItemVM.Empty.End(RangeEnd.Before, focusedCount = beforeFocusedCount, count = beforeCount, onClearRange = onClearRange)
    }else{
        null
    }
    val mappedList : List<FocusCommandItemVM> = list.map {
        return@map when(it){
            is FocusedCommandOrBreak.Break -> FocusCommandItemVM.Empty.Break(it.index, it.count, onClearFocus)
            is FocusedCommandOrBreak.Focused -> FocusCommandItemVM.Content(it.serializableCommandStateAndContext, onItemWithIdClicked)
        }
    }

    val after = if(afterCount > 0){
        FocusCommandItemVM.Empty.End(RangeEnd.After, focusedCount = afterFocusedCount, count = afterCount, onClearRange = onClearRange)
    }else{
        null
    }
